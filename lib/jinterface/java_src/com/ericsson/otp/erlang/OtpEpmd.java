...
public class OtpEpmd {
	...
	try {
	    final OtpOutputStream obuf = new OtpOutputStream();
	    s = new Socket(node.hostAddress(), EpmdPort.get());
		...
		throw new IOException("Nameserver not responding on "
			+ node.hostAddress() + " when looking up " + node.alive());
	    }
	    ...
	    throw new IOException("Nameserver not responding on " + node.hostAddress()
	...
	try {
		...
		throw new IOException("Nameserver not responding on "
			+ node.hostAddress() + " when publishing " + node.alive());
	    }
